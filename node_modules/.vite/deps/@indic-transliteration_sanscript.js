...
//#region node_modules/@indic-transliteration/sanscript/sanscript.js
var require_sanscript = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	schemes.baraha = {
		"_vowel_comment": [
			"omissions  from optitrans_dravidian",
			"ea",
			"oa"
		],
		"_yogavaahas_comment": [
	...
	function exportSanscriptSingleton(global, schemes, devanagariVowelToMarks) {
		...
		var DETECTION_PATTERNS = {
			...
			RE_KOLKATA_ONLY: /[ēō]/,
			RE_SLP1_ONLY: RegExp([
				"[fFxXEOCYwWqQPB]|kz|Nk|Ng|tT|dD|Sc|Sn|",
				"[aAiIuUfFxXeEoO]R|",
				"G[yr]|(\\W|^)G"
			].join("")),
		...
		Sanscript.detect = function(text) {
			...
			for (var _i = 0; _i < text.length; _i++) {
				...
				if (code >= DETECTION_PATTERNS.BRAHMIC_FIRST_CODE_POINT && code <= DETECTION_PATTERNS.BRAHMIC_LAST_CODE_POINT) for (var j = 0; j < BLOCKS.length; j++) {
					var block = BLOCKS[j];
					if (code >= block[1]) return block[0];
				}
			}
			if (DETECTION_PATTERNS.RE_IAST_OR_KOLKATA_ONLY.test(text)) {
				if (DETECTION_PATTERNS.RE_KOLKATA_ONLY.test(text)) return Scheme.Kolkata;
				return Scheme.IAST;
			}
			if (DETECTION_PATTERNS.RE_ITRANS_ONLY.test(text)) return Scheme.ITRANS;
			if (DETECTION_PATTERNS.RE_SLP1_ONLY.test(text)) return Scheme.SLP1;
			if (DETECTION_PATTERNS.RE_VELTHUIS_ONLY.test(text)) return Scheme.Velthuis;
			if (DETECTION_PATTERNS.RE_ITRANS_OR_VELTHUIS_ONLY.test(text)) return Scheme.ITRANS;
			return Scheme.HK;
		};
		...
		Sanscript.addRomanScheme = function(name, scheme) {
			if (!("vowel_marks" in scheme)) {
				...
				for (var _i2 = 0, _Object$entries = Object.entries(scheme.vowels); _i2 < _Object$entries.length; _i2++) {
					...
					if (key != "अ") scheme.vowel_marks[devanagariVowelToMarks[key]] = value;
				}
			}
			Sanscript.schemes[name] = scheme;
		...
		(function() {
			...
			for (var _i3 = 0, _Object$entries2 = Object.entries(schemes); _i3 < _Object$entries2.length; _i3++) {
				...
				if (scheme.isRomanScheme) Sanscript.addRomanScheme(schemeName, scheme);
			}
		})();
		var makeMap = function makeMap(from, to, _options) {
			...
			for (var group in fromScheme) {
				if (!{}.hasOwnProperty.call(fromScheme, group)) continue;
				if ([
					"alternates",
					"accented_vowel_alternates",
					"isRomanScheme"
				...
				for (var _i4 = 0, _Object$entries3 = Object.entries(fromGroup); _i4 < _Object$entries3.length; _i4++) {
					...
					for (j = 0; j < numAlts; j++) tokenLengths.push(alts[j].length);
					if (group === "vowel_marks" || group === "virama") {
						marks[F] = T;
						for (j = 0; j < numAlts; j++) marks[alts[j]] = T;
					} else {
						letters[F] = T;
						for (j = 0; j < numAlts; j++) letters[alts[j]] = T;
						if (group === "consonants" || group === "extra_consonants") {
							consonants[F] = T;
							for (j = 0; j < numAlts; j++) consonants[alts[j]] = T;
						}
						if (group === "accents") {
							accents[F] = T;
							for (j = 0; j < numAlts; j++) accents[alts[j]] = T;
						}
					}
				}
			}
			if (fromScheme["accented_vowel_alternates"]) for (var _i5 = 0, _Object$keys = Object.keys(fromScheme["accented_vowel_alternates"]); _i5 < _Object$keys.length; _i5++) {
				...
				try {
					for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
						...
						if (!letters[baseVowel]) console.error(baseVowel, targetAccent, letters);
		...
		var transliterateRoman = function transliterateRoman(data, map, options) {
			...
			for (var i = 0, L; (L = data.charAt(i)) || tokenBuffer; i++) {
				...
				for (var j = 0; j < maxTokenLength; j++) {
					...
					if (skippingSGML === true) skippingSGML = token !== ">";
					else if (token === "<") skippingSGML = optSkipSGML;
					else if (token === "##") {
					...
					if ((tempLetter = letters[token]) !== void 0 && !skippingTrans) {
						if (toRoman) buf.push(tempLetter);
						else {
							if (hadConsonant) {
								if (tempMark = marks[token]) buf.push(tempMark);
								else if (token !== map.fromSchemeA) {
									buf.push(virama);
									buf.push(tempLetter);
								}
							} else buf.push(tempLetter);
		...
		var transliterateBrahmic = function transliterateBrahmic(data, map, _options) {
			...
			if (hadRomanConsonant) buf.push(map.toSchemeA);
			return buf.join("");
		};
		Sanscript.t = function(data, from, to, options) {
			if (!from) from = Sanscript.detect(data).toLowerCase();
			options = options || {};
			...
			if (hasPriorState) map = cache.map;
			else {
			...
			if (fromShortcuts) for (var _key in fromShortcuts) {
				var shortcut = fromShortcuts[_key];
				if (_key.includes(shortcut)) data = data.replace(_key, shortcut);
				data = data.replace(shortcut, _key);
			}
			var result = "";
			if (map.fromRoman) result = transliterateRoman(data, map, options);
			else result = transliterateBrahmic(data, map, options);
			var toShortcuts = schemes[to]["shortcuts"];
			if (toShortcuts) for (var _key2 in toShortcuts) {
				var _shortcut = toShortcuts[_key2];
				if (_shortcut.includes(_key2)) result = result.replace(_shortcut, _key2);
			...
			if (_typeof(options.preferred_alternates[to]) == "object") {
				...
				for (var i = 0; i < keys.length; i++) result = result.split(keys[i]).join(options.preferred_alternates[to][keys[i]]);
			}
			return result;
		};
		Sanscript.transliterateWordwise = function(data, from, to, options) {
			options = options || {};
			return data.split(/\s+/).map(function(word) {
				return [word, Sanscript.t(word, from, to, options)];
			});
		};
		...
		} else global.Sanscript = Sanscript;
	}
	exportSanscriptSingleton(exports, schemes, devanagariVowelToMarks);
}));
//#endregion
export default require_sanscript();
